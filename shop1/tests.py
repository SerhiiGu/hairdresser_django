...
class TestBooking(TestCase):
    def test_booking(self):
        ...
        master1.save()
        calendar1 = Calendar(master=master1, date="2023-04-15 00:00:00", time_start="11:00", time_end="13:00")
        calendar1.save()
    # Просте резервування
        c = Client()
        response = c.post(f'/booking/{master1.id}/{service1.id}/', {"date": "2023-04-15 11:30"})
        self.assertEqual(response.status_code, 200)
        booking = Booking.objects.filter(master=master1, service=service1, date="2023-04-15 11:30")
        self.assertEqual(len(booking), 1)
    # Два резервування на різні проміжки
        response = c.post(f'/booking/{master1.id}/{service1.id}/', {"date": "2023-04-15 11:00"})
        self.assertEqual(response.status_code, 200)
        booking = Booking.objects.filter(master=master1, service=service1, date="2023-04-15 11:00")
        self.assertEqual(len(booking), 1)

        response = c.post(f'/booking/{master1.id}/{service1.id}/', {"date": "2023-04-15 12:15"})
        self.assertEqual(response.status_code, 200)
        ...
        master2.save()
        calendar2 = Calendar(master=master2, date="2023-04-16 00:00:00", time_start="11:00", time_end="13:00")
        calendar2.save()
    ...
    # проте один довго думав(сторінка була відкрита і не перезавантажувалася), і за цей час частину слоту зайняли
        c = Client()
        response = c.post(f'/booking/{master2.id}/{service2.id}/', {"date": "2023-04-16 11:30"})
        self.assertEqual(response.status_code, 200)
        booking = Booking.objects.filter(master=master2, service=service2, date="2023-04-16 11:30")
        self.assertEqual(len(booking), 1)

        response = c.post(f'/booking/{master2.id}/{service3.id}/', {"date": "2023-04-16 11:45"})
